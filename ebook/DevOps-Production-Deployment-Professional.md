...
**⚠️ Important**: You also need Docker Desktop for the container
runtime:
...
**⚠️ Resource Requirements**: Ensure you have at least 4GB available
...
**⚠️ IMPORTANT**: This project uses `gameapp.games` as an example
...
**⚠️ Important**: The `(healthy)` status for frontend indicates Docker
...
**⚠️ Warning**: The `-v` flag removes persistent volumes, deleting all
database data.
...
**⚠️ Important**: Ensure your `k8s/secrets.yaml` has been created from
...
- Password: (from previous command) - **⚠️ Accept the self-signed
certificate warning**
